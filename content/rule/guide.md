...
Wilddog提供了一种声明式的语言来编写“安全规则”，这些规则在Wilddog云端保护App数据。你可以进入Wilddog的控制面板，选择一个应用进行管理，在“安全规则”中编辑这个规则。
通过这个安全规则，你可以控制任何一个数据节点的访问权限。在任何一个数据节点上适用的规则，将会自动延展到所有子节点。
...
Wilddog的安全规则中包含一些内置的变量和函数，可用于构建安全规则表达式。这些变量和函数功能强大，使得表达式中可以引用其它数据节点，云端时间戳等。
...
### 数据校验
Wilddog是一个schema-free的数据库。但是应用数据的一致性是很重要的。 安全规则引擎中包含一个`.validate`规则。它的使用和`.read`，`.write`相同，唯一的区别是`.validate`规则不会自动延展到子节点。
...
```
如果没有为指定的节点赋予读和写的安全规则（如上边代码中数据的根节点），那么它们将会采用默认值false。

### 内置变量
在安全规则中，可以使用一些内置变量。下面的实例中将会涵盖到大部分内置变量，这里先列出一个简要描述：
| 变量 | 描述 |
| --- | --- |
| now | 当前的时间戳，以毫秒作为单位 |
...
### 规则重叠
这是一个非常重要的特性！安全规则遵循一个自上向下延展的原则。如果一个数据节点上的`.write`或`.read`规则执行结果为true时，同时它的所有的子节点也都将拥有读或写的权限。子节点上的规则不能收回父节点或祖先节点已经赋予的读或写的权限。参考下面的例子：
``` js
